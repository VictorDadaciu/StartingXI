...
{  
    namespace detail
    {
        ...
        constexpr decltype(auto) tupleApplyImpl(Func&& func, TTuple&& tuple, std::index_sequence<Indices...>) noexcept
        {
            return SXI_MPL_FWD(func)(std::get<Indices>(SXI_MPL_FWD(tuple))...);
        }

        template <typename Func, typename TTuple>
        constexpr decltype(auto) tupleApply(Func&& func, TTuple&& tuple) noexcept
        {
        ...
        constexpr decltype(auto) forArgs(Func&& func, Ts&&... args) noexcept
        {
            return (void)std::initializer_list<int>{(func(SXI_MPL_FWD(args)), 0)...};
        }

        template <typename Func, typename TTuple>
        constexpr decltype(auto) forTuple(Func&& func, TTuple&& tuple) noexcept
        {
            return tupleApply(
                [&func](auto&&... xs)
                {
    ...
    constexpr decltype(auto) forTuple(
        Func&& func, TTuple&& tuple) noexcept
    {
