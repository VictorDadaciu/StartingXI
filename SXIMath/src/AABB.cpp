...
bool AABB::intersects(const AABB& other) const
{
    ...
    return fabsf((topLeft.x + halfS.x) - (other.topLeft.x + oHalfS.x)) <=
           (halfS.x + oHalfS.x) &&
           fabsf((topLeft.y + halfS.y) - (other.topLeft.y + oHalfS.y)) <=
           (halfS.y + oHalfS.y);
}
...
{
    return AABB(std::fminf(a.topLeft.x, b.topLeft.x),
                std::fminf(a.topLeft.y, b.topLeft.y),
                std::fmaxf(a.botRight.x, b.botRight.x),
                std::fmaxf(a.botRight.y, b.botRight.y));
}
} // namespace sxi