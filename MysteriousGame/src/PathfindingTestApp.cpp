...
// std::chrono::duration<float>(1.f/60); static
...
//
//     PathPoint(float x, float y, const std::array<unsigned int, 4>& mk) :
//     pos(x, y), moveKeys(mk) {} PathPoint(glm::vec2&& p, const
//     std::array<unsigned int, 4>& mk) : pos(p), moveKeys(mk) {}
//
//     void update()
//     {
...
//             pos.y += speed * dt.count();
//         }
//         pos = glm::clamp(pos, glm::vec2(X_1, Y_1) + 5.f, glm::vec2(X_2, Y_2)
//         - 5.f);
//     }
//
// } start(80, 80, { SDLK_W, SDLK_A, SDLK_S, SDLK_D }), goal(650, 1000,
...
//     std::sort(aabbs.begin(), aabbs.end(), [](std::pair<sxi::AABB, int8_t>
//     left, std::pair<sxi::AABB, int8_t> right) { return left.second >
//     right.second; });
// }
//
...
//     }
//
//     if (!SDL_CreateWindowAndRenderer("MapState", Y_RES + 30, X_RES + 30, 0,
//     &window, &renderer))
//     {
...
//             sxi::Map::instance().insert(sxi::ShapeType::Default,
//             shapeUnderConstruction); shapeUnderConstruction.clear();
//             updateMap();
//         }
//
...
//             continue;
//
//         SDL_SetRenderDrawColor(renderer, colors[3 * aabb.second + 0],
//         colors[3 * aabb.second + 1], colors[3 * aabb.second + 2],
//         SDL_ALPHA_OPAQUE); SDL_RenderLine(renderer, Y_BUFFER +
//         aabb.first.topLeft.y, X_BUFFER + aabb.first.topLeft.x, Y_BUFFER +
//         aabb.first.botRight.y, X_BUFFER +aabb.first.topLeft.x);
//         SDL_RenderLine(renderer, Y_BUFFER + aabb.first.botRight.y, X_BUFFER +
//         aabb.first.topLeft.x, Y_BUFFER + aabb.first.botRight.y, X_BUFFER
//         +aabb.first.botRight.x); SDL_RenderLine(renderer, Y_BUFFER +
//         aabb.first.botRight.y, X_BUFFER + aabb.first.botRight.x, Y_BUFFER +
//         aabb.first.topLeft.y, X_BUFFER +aabb.first.botRight.x);
//         SDL_RenderLine(renderer, Y_BUFFER + aabb.first.topLeft.y, X_BUFFER +
//         aabb.first.botRight.x, Y_BUFFER + aabb.first.topLeft.y, X_BUFFER
//         +aabb.first.topLeft.x);
//     }
// }
//
...
//         SDL_RenderLine(renderer, sy1, sx1, sy2, sx2);
//     }
//     SDL_FRect rectStart = { Y_BUFFER + start.pos.y - 5, X_BUFFER +
//     start.pos.x - 5, 10, 10 }; SDL_FRect rectGoal = { Y_BUFFER + goal.pos.y -
//     5, X_BUFFER + goal.pos.x - 5, 10, 10 }; SDL_SetRenderDrawColor(renderer,
//     120, 12, 120, SDL_ALPHA_OPAQUE); SDL_RenderFillRect(renderer,
//     &rectStart); SDL_SetRenderDrawColor(renderer, 12, 120, 12,
//     SDL_ALPHA_OPAQUE); SDL_RenderFillRect(renderer, &rectGoal);
// }
//
