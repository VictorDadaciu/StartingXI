...
public:
    ...
    void collect(std::vector<glm::vec2>&,
                 std::vector<bool>&,
                 std::vector<int>&,
                 bool) const;
...
private:
    QuarterEdge*
    insert(MapPoint*, QuarterEdge*, std::unordered_set<QuarterEdge*>&);
    ...
    QuarterEdge*
    forceConnect(MapPoint*, MapPoint*, std::unordered_set<QuarterEdge*>&) const;
    ...
    {
        return isBoundaryPoint(edge->data->v) ||
               isBoundaryPoint(edge->sym->data->v);
    }
    ...
    inline bool insideCircumspectCircle(MapPoint* a,
                                        MapPoint* b,
                                        MapPoint* c,
                                        MapPoint* d) const
    {
