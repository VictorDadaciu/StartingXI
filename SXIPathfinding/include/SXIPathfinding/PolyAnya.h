...
class PolyAnya
{
    ...
    public:
        ...
        inline bool operator<(const PNode& other) const { return f < other.f; }

        inline bool operator>(const PNode& other) const { return f > other.f; }

        inline bool includeA() const { return inclusion & 0xFFFF0000; }

        inline bool includeB() const { return inclusion & 0x0000FFFF; }
    };
...
std::vector<glm::vec2> runPolyAnya(const glm::vec2&,
                                   const glm::vec2&,
                                   QuarterEdge*,
                                   QuarterEdge*,
                                   PAMetadata&);
