...
{
    if (!initialized)
        throw std::runtime_error("Must initialize map before inserting points");
...
{
    if (!initialized)
        throw std::runtime_error("Must initialize map before removing points");
...
void Map::collectCDT(std::vector<glm::vec2>& points,
