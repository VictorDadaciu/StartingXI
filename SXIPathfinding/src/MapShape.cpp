...
namespace sxi
{
MapShape::MapShape(const std::vector<QuarterEdge*>& edges) : edges(edges)
{
    const std::unordered_set<QuarterEdge*> outerEdges(edges.begin(),
                                                      edges.end());
    ...
    for (const QuarterEdge* edge : edges)
    {
        ...
        if (outerEdges.find(lnext) ==
            outerEdges.end() &&
            innerEdgesAlreadySeen.find(lnext) == innerEdgesAlreadySeen.end())
        {
            ...
            lnext = lnext->sym;
            if (innerEdgesAlreadySeen.find(lnext) ==
                innerEdgesAlreadySeen.end())
            {
                internals.emplace_back(lnext);
                innerEdgesAlreadySeen.insert(lnext);
                QuarterEdge* ptr = lnext->sym->prevOn();
...
QuarterEdge* MapShape::closestEdgeForPointInside(const glm::vec2& point,
                                                 glm::vec2& newPoint) const
{
