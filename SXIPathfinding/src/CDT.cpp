                               ...
                               std::unordered_set<QuarterEdge*>& toCull) const
{
    ...
    if (!retVal)
        throw std::runtime_error("Could not create a constrained edge");
                          ...
                          QuarterEdge* bestEdge)
{
    ...
    int n = points.size();
    if (n-- <= 2)
        throw std::runtime_error("Cannot make shape from 2 or fewer MapPoints");
...
void CDT::collect(std::vector<glm::vec2>& points,
