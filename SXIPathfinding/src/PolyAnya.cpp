...
namespace sxi
{
PolyAnya::PNode::PNode(const glm::vec2& root,
                       float g,
                       const glm::vec2& a,
                       bool includeA,
                       const glm::vec2& b,
                       bool includeB,
                       QuarterEdge* edge,
                       glm::vec2 goal) : root(root), a(a), b(b), edge(edge)
{
    ...
    float h = ((rootGoal.between(rayA, rayB))
                   ? glm::length(rootGoal.dir)
                   : fminf(glm::length(rayA.dir) + glm::length(goal - a),
                           glm::length(rayB.dir) + glm::length(goal - b)));
    f = g + h;
}
...
PolyAnya::PolyAnya(const glm::vec2& start,
                   const glm::vec2& goal,
                   QuarterEdge* startEdge,
                   QuarterEdge* goalEdge) : start(start), goal(goal)
{
    gScores[start] = 0;
...
void PolyAnya::emplace(const glm::vec2& root,
                       const glm::vec2& a,
                       bool includeA,
                       const glm::vec2& b,
                       bool includeB,
                       QuarterEdge* edge,
                       bool enteringGoalShape)
{
    if (enteringGoalShape)
    {
        ...
        {
            open.emplace(
                goal, gScores[goal], goal, false, goal, false, nullptr, goal);
        }
    }
    else
    {
...
std::vector<glm::vec2> PolyAnya::run()
{
    while (!open.empty())
    {
        ...
        {
            if (!ptr->constrained ||
                enteringGoalShape) // impassable edges are uninteresting for
                                   // expansion, unless goal shape
            {
                glm::vec2 dataRoot =
                    ptr->data->v - node.root; // root -> beginning of edge
                glm::vec2 symRoot =
                    ptr->sym->data->v - node.root; // root -> end of edge
                ...
                bool dataBetween = !dataLeftOfCone &&
                                   !dataOnLeft &&
                                   !dataRightOfCone &&
                                   !dataOnRight;
                bool symBetween = !symLeftOfCone &&
                                  !symOnLeft &&
                                  !symRightOfCone &&
                                  !symOnRight;
                glm::vec2 intA, intB;
                ...
                if ((dataLeftOfCone || dataOnLeft) &&
                    (symLeftOfCone || symOnLeft))
                {
                    if (includeA)
                        emplace(node.a,
                                ptr->data->v,
                ...
                else if ((dataOnLeft || dataBetween) &&
                         (symOnRight || symBetween))
                {
                    emplace(node.root,
                            ptr->data->v,
                            true,
                ...
                else if ((dataRightOfCone || dataOnRight) &&
                         (symRightOfCone || symOnRight))
                {
                    if (includeB)
                        emplace(node.b,
                                ptr->data->v,
...
std::vector<glm::vec2> runPolyAnya(const glm::vec2& start,
                                   const glm::vec2& goal,
                                   QuarterEdge* startEdge,
                                   QuarterEdge* goalEdge,
                                   PAMetadata& metadata)
{
    ...
    {
        if (ptr == goalEdge)
            return std::vector<glm::vec2>{start, goal};
        ptr = ptr->sym->prevOn();
    } while (ptr != startEdge);
