...
void RenderPass::createRenderPass()
{
    ...
    if (vkCreateRenderPass(
            context->logicalDevice, &renderPassInfo, nullptr, &pass) !=
        VK_SUCCESS)
...
void RenderPass::createFrameBuffers()
{
    ...
    for (size_t i = 0; i < frameBuffers.size(); ++i)
    {
        std::array<VkImageView, 3> attachments = {
            colorImageView, depthImageView, window->swapchain->imageViews[i]};
        ...
        if (vkCreateFramebuffer(context->logicalDevice,
                                &framebufferInfo,
                                nullptr,
