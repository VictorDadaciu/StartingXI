...
u32 findMemoryType(u32 typeFilter, VkMemoryPropertyFlags properties)
{
    ...
    vkGetPhysicalDeviceMemoryProperties(context->currentPhysicalDevice().device,
                                        &memProperties);
                             ...
                             VkFormatFeatureFlags features)
{
    for (VkFormat format : candidates)
    {
        ...
        vkGetPhysicalDeviceFormatProperties(
            context->currentPhysicalDevice().device, format, &props);
                  ...
                  VkDeviceMemory& mem)
{
    ...
    if (vkCreateBuffer(context->logicalDevice, &bufferInfo, nullptr, &buffer) !=
        VK_SUCCESS)
        throw ResourceCreationException("Failed to create buffer");

    VkMemoryRequirements memRequirements;
    vkGetBufferMemoryRequirements(
        context->logicalDevice, buffer, &memRequirements);
    ...
    if (vkAllocateMemory(context->logicalDevice, &allocInfo, nullptr, &mem) !=
        VK_SUCCESS)
                 ...
                 VkDeviceMemory& imageMem)
{
    ...
    if (vkCreateImage(context->logicalDevice, &imageInfo, nullptr, &image) !=
        VK_SUCCESS)
        throw ResourceCreationException("Failed to create image");

    VkMemoryRequirements memRequirements;
    vkGetImageMemoryRequirements(
        context->logicalDevice, image, &memRequirements);
    ...
    if (vkAllocateMemory(
            context->logicalDevice, &allocInfo, nullptr, &imageMem) !=
        VK_SUCCESS)
                            ...
                            u32 mipLevels)
{
    ...
    if (vkCreateImageView(
            context->logicalDevice, &viewInfo, nullptr, &imageView) !=
        VK_SUCCESS)
...
void generateMipmaps(
    VkImage image, VkFormat format, i32 texWidth, i32 texHeight, u32 mipLevels)
{
    const PhysicalDevice& physicalDevice = context->currentPhysicalDevice();
    VkFormatProperties formatProperties;
    vkGetPhysicalDeviceFormatProperties(
        physicalDevice.device, format, &formatProperties);
...
VkCommandBuffer beginSingleTimeCommands()
{
    ...
    vkAllocateCommandBuffers(
        context->logicalDevice, &allocInfo, &commandBuffer);
...
void endSingleTimeCommands(const VkCommandBuffer& commandBuffer)
{
    ...
    vkFreeCommandBuffers(context->logicalDevice,
                         context->currentFrameContext()->commandPool,
                         1,
                         &commandBuffer);
}
...
VkShaderModule createShaderModule(const std::vector<char>& shaderCode)
{
    ...
    if (vkCreateShaderModule(
            context->logicalDevice, &createInfo, nullptr, &shaderModule) !=
        VK_SUCCESS)
