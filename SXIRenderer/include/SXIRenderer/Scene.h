...
public:
    ...
    void run(ecs::Manager<TSettings> &mgr, const Time &time)
    {
        ...
        mgr.template forEntitiesMatching<
            ecs::Signature<ecs::PositionComponent, ecs::LightTag>>(
            [this, &time, &currentFrame](auto &, auto &posComponent)
            {
                ...
                posComponent.pos = glm::vec3(100.f * std::sinf(timePassed),
                                             30,
                                             100.f * std::cosf(timePassed));

                char *offset =
                    (char *)detail::uniformBuffers[currentFrame].mapped;

                this->frameUBO.view = glm::lookAt(glm::vec3(0.f, 50.f, -75.f),
                                                  glm::vec3(0.f, 20.f, 0.f),
                                                  SXI_VEC3_UP);
                this->frameUBO.proj = glm::perspective(
                    glm::radians(60.0f),
                ...
                this->frameLight = FrameLight{glm::vec4(posComponent.pos.x,
                                                        posComponent.pos.y,
                                                        posComponent.pos.z,
                                                        1.f)};
        ...
        mgr.template forEntitiesMatching<ecs::Signature<ecs::PositionComponent,
                                                        ecs::YRotationComponent,
                                                        ecs::RenderComponent>>(
            [this](auto &entityIndex,
                   auto &posComponent,
                   auto &rotComponent,
                   auto &)
            {
                this->objectUBOs[entityIndex].model = glm::rotate(
                    glm::translate(glm::mat4(1.0f), posComponent.pos),
                    rotComponent.rot,
                    SXI_VEC3_UP);
            });
        memcpy(
            offset, objectUBOs.data(), objectUBOs.size() * sizeof(ObjectUBO));
    }

    inline const SceneData &currentSceneData() const
    {
        return sceneDatas[detail::context->currentFrame()];
    }
