...
namespace sxi
{
	...
	Renderer::Renderer(std::vector<const char*>&& extensions)
	{
	...
	void Renderer::cleanupSwapChain() const
	{
		...
		vkFreeMemory(logicalDevice, colorImageMem, nullptr);

		vkDestroyImageView(logicalDevice, depthImageView, nullptr);
		vkDestroyImage(logicalDevice, depthImage, nullptr);
		vkFreeMemory(logicalDevice, depthImageMem, nullptr);
	...
	void Renderer::cleanup() const
	{
		...
		for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) 
		{
			...
			vkFreeMemory(logicalDevice, uniformBuffersMem[i], nullptr);
		}
	...
	void Renderer::initialize(Window* window, std::vector<char>&& vertCode, std::vector<char>&& fragCode, Model* model)
	{
		this->model = model;
		this->window = window;
	...
	void Renderer::createInstance(std::vector<const char*>&& extensions)
	{
		if (enableValidationLayers && !checkValidationLayerSupport())
			throw std::exception("Vulkan validation layers not supported by installed version");
	...
	void Renderer::setupDebugCallback() const
	{
		VkDebugUtilsMessengerCreateInfoEXT createInfo{};
		populateDebugMessengerCreateInfo(createInfo);

		if (createDebugUtilsMessengerEXT(instance, &createInfo, nullptr, &debugMessenger) != VK_SUCCESS)
			throw std::exception("Failed to create debug messenger");
	}
	...
	int Renderer::physicalDeviceScore(const VkPhysicalDevice device) const
	{
	...
	void Renderer::choosePhysicalDevice()
	{
		...
		if (deviceCount == 0)
			throw std::exception("No physical device found");
	...
	void Renderer::createLogicalDevice()
	{
		...
		if (vkCreateDevice(physicalDevice, &createInfo, nullptr, &logicalDevice) != VK_SUCCESS)
			throw std::exception("Failed to create physical device");
	...
	VkSurfaceFormatKHR Renderer::chooseSwapChainSurfaceFormat(const std::vector<VkSurfaceFormatKHR>& availableFormats) const
	{
	...
	VkPresentModeKHR Renderer::chooseSwapChainPresentMode(const std::vector<VkPresentModeKHR>& availablePresentModes) const
	{
	...
	VkExtent2D Renderer::chooseSwapChainExtent(const VkSurfaceCapabilitiesKHR& capabilities) const
	{
		...
		{
			int width, height;
			window->sizeInPixels(width, height);
	...
	void Renderer::createGraphicsPipeline(std::vector<char>&& vertCode, std::vector<char>&& fragCode)
	{
		VkShaderModule vertModule = createShaderModule(std::move(vertCode));
		VkShaderModule fragModule = createShaderModule(std::move(fragCode));
	...
	VkShaderModule Renderer::createShaderModule(std::vector<char>&& shaderCode) const
	{
	...
	void Renderer::createDepthResources()
	{
		...
		createImage(swapChainExtent.width, swapChainExtent.height, 1, msaaSamples, depthFormat, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, depthImage, depthImageMem);
	...
	void Renderer::createColorResources()
	{
		...
		createImage(swapChainExtent.width, swapChainExtent.height, 1, msaaSamples, colorFormat, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSIENT_ATTACHMENT_BIT | VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, colorImage, colorImageMem);
		colorImageView = createImageView(colorImage, colorFormat, VK_IMAGE_ASPECT_COLOR_BIT, 1);
	}

	void Renderer::createImage(u32 width, u32 height, u32 mipLevels, VkSampleCountFlagBits numSamples, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage& image, VkDeviceMemory& imageMem)
	{
	...
	VkImageView Renderer::createImageView(VkImage image, VkFormat format, VkImageAspectFlags aspectFlags, u32 mipLevels) const
	{
	...
	void Renderer::createTextureImage()
	{
		...
		const Texture* image = model->texture();
		createBuffer(image->size(), VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, stagingBuffer, stagingBufferMem);

		void* data;
		vkMapMemory(logicalDevice, stagingBufferMem, 0, image->size(), 0, &data);
		memcpy(data, image->raw(), SXI_TO_SIZE(image->size()));
		...
		transitionImageLayout(textureImage, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, image->mipLevels());
		copyBufferToImage(stagingBuffer, textureImage, SXI_TO_U32(image->width()), SXI_TO_U32(image->height()));
		generateMipmaps(textureImage, VK_FORMAT_R8G8B8A8_SRGB, SXI_TO_I32(image->width()), SXI_TO_I32(image->height()), image->mipLevels());

		vkDestroyBuffer(logicalDevice, stagingBuffer, nullptr);
		vkFreeMemory(logicalDevice, stagingBufferMem, nullptr);
	}

	void Renderer::generateMipmaps(VkImage image, VkFormat format, i32 texWidth, i32 texHeight, u32 mipLevels)
	{
	...
	{
		textureImageView = createImageView(textureImage, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_ASPECT_COLOR_BIT, model->texture()->mipLevels());
	}

	void Renderer::createTextureImageSampler()
	{
	...
	void Renderer::createBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags memProperties, VkBuffer& buffer, VkDeviceMemory& mem)
	{
		...
		if (vkAllocateMemory(logicalDevice, &allocInfo, nullptr, &mem) != VK_SUCCESS)
	...
	void Renderer::transitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout, u32 mipLevels)
	{
		...
		{
			throw std::exception("Unsupported layout transition");
		}
	...
	{
		const std::vector<Vertex> verts = model->vertices();
		...
		void* data;
		vkMapMemory(logicalDevice, stagingBufferMem, 0, bufferSize, 0, &data);
	...
	void Renderer::createIndexBuffer()
	{
		...
		createBuffer(bufferSize, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_INDEX_BUFFER_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, indexBuffer, indexBufferMem);
	...
	void Renderer::createUniformBuffers()
	{
		...
		for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
			createBuffer(bufferSize, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, uniformBuffers[i], uniformBuffersMem[i]);

			vkMapMemory(logicalDevice, uniformBuffersMem[i], 0, bufferSize, 0, &uniformBuffersMapped[i]);
		}
	}
	...
	void Renderer::createDescriptorPool()
	{
		...
		poolSizes[0].descriptorCount = SXI_TO_U32(MAX_FRAMES_IN_FLIGHT);
		poolSizes[1].type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
		poolSizes[1].descriptorCount = SXI_TO_U32(MAX_FRAMES_IN_FLIGHT);

		VkDescriptorPoolCreateInfo poolInfo{};
		poolInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_POOL_CREATE_INFO;
		poolInfo.poolSizeCount = SXI_TO_U32(poolSizes.size());
		poolInfo.pPoolSizes = poolSizes.data();
		poolInfo.maxSets = SXI_TO_U32(MAX_FRAMES_IN_FLIGHT);
	...
	void Renderer::createDescriptorSets()
	{
		...
		allocInfo.descriptorSetCount = SXI_TO_U32(MAX_FRAMES_IN_FLIGHT);
		...
		for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++)
		{
			...
			vkUpdateDescriptorSets(logicalDevice, SXI_TO_U32(descriptorWrites.size()), descriptorWrites.data(), 0, nullptr);
		}
	}
	...
	void Renderer::createCommandBuffer()
	{
		...
		allocInfo.commandPool = commandPool;
		allocInfo.level = VK_COMMAND_BUFFER_LEVEL_PRIMARY;
		allocInfo.commandBufferCount = SXI_TO_U32(commandBuffers.size());

		if (vkAllocateCommandBuffers(logicalDevice, &allocInfo, commandBuffers.data()) != VK_SUCCESS)
			throw std::exception("Failed to allocate command buffers");
	}

	void Renderer::recordCommandBuffer(VkCommandBuffer commandBuffer, u32 imageIndex, u32 currentFrame)
	{
		VkCommandBufferBeginInfo beginInfo{};
		beginInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO;
		beginInfo.flags = 0;
	...
	void Renderer::render(const Time& time)
	{
		...
		updateUniformBuffers(time, currentFrame);
